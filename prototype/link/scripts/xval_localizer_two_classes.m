...
function [ subj results] = xval_localizer_two_classes( subj_id, neuropipe_subj_dir, varargin )
...
% TODO: The naming conventions for patterns etc is extremely fragile and
% idiosyncratic - it might be worthwhile 
...
regressors_suffix = [num2str(options.img_localizer_idcs(1)) '_' num2str(options.img_localizer_idcs(2))];
...
% 	subj = set_mat(subj,'regressors','conds',get_img_localizer_regressors_and_concatenate(runs,options.img_localizer_idcs));
	...
	subj = set_mat(subj,'regressors',regressor_name,get_img_localizer_regressors_and_concatenate(runs,options.img_localizer_idcs));
end
